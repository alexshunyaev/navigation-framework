...
\paragraph{Limitations and future work.} The environment is static and fully observed, so
dynamic obstacles and true partial observability are not yet handled, and DWA still needs
manual weight tuning and stays freeze-prone. Natural next steps: online replanning when the
map changes, a sampling-based global planner (RRT*) for continuous spaces, learning the cost
weights instead of hand-tuning, and an explicit recovery behaviour for the frozen-robot state.
